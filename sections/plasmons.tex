...
\begin{document}
    ...
    Стоит отметить, что именно этот параметр определяет коэффициент поглощения $\alpha (\omega)= 2 \Im q (\omega)$ при $\omega \in \mathbb{R}$,
    поскольку считая мнимую часть частоты малой  $\Re \omega \gg \Im \omega$ за счёт аналитическорго продолжения функции $q(\omega)$ можно получить: 
    \begin{equation*}
        q(\omega \in \mathbb{R}) \approx q(\Re \omega + i \Im \omega) \approx \underbrace{q(\Re \omega)}_{\Re q} + 
            i \underbrace{\Im \omega \left.\frac{\partial q}{\partial \omega} \right\vert{}_{\Re \omega}}_{\alpha / 2  = \Im q};
    \end{equation*}
